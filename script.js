...
function initVoice() {
    ...
    // Ưu tiên Microsoft An (giọng chuẩn Windows), sau đó là các giọng vi-VN khác
...
function generateQuestionHTML(idx, rawSearch = "") {
    ...
    q.options.forEach((opt,optIdx)=>{
        ...
        if(isSubmitted && correctIndices.includes(optIdx)) { extraClass="bg-green-100 border-green-300"; badge=true; } else if(isSubmitted && isChecked && !correctIndices.includes(optIdx)) extraClass="bg-red-100 border-red-300";
        // Voice icon for option
        const voiceOptBtn = `<i class="fas fa-volume-up voice-btn ml-auto text-gray-400 hover:text-blue-500" onclick="event.preventDefault(); speak(\`${escapeHtml(opt).replace(/['"`]/g, '')}\`)"></i>`;
    ...
    let feedback='';
    if(isSubmitted && !isCorrect) feedback=`<div class="mt-3 text-sm bg-amber-50 p-2 rounded-lg"><i class="fas fa-info-circle"></i> Đáp án đúng: ${escapeHtml(correctIndices.map(i=>q.options[i]).join('; '))}</div>`; else if(isSubmitted && isCorrect) feedback=`<div class="mt-3 text-sm text-green-700"><i class="fas fa-check-circle"></i> Chính xác!</div>`;
    ...
    // Voice icon for question
    const voiceQBtn = `<button class="voice-btn text-gray-400 hover:text-blue-500" onclick="speak(\`${escapeHtml(q.text).replace(/['"`]/g, '')}\`)" title="Nghe câu hỏi"><i class="fas fa-volume-up"></i></button>`;
    const aiExplainBtn = `<button class="ai-explain-btn text-xs bg-purple-50 hover:bg-purple-100 text-purple-700 px-2 py-1 rounded border border-purple-200 transition" data-qidx="${idx}"><i class="fas fa-magic"></i> Giải thích AI</button>`;
    ...
    return `<div id="question-${idx}" class="${cardClass}"><div class="p-5"><div class="flex justify-between items-start gap-2 flex-wrap"><h3 class="font-semibold text-gray-800 text-base flex gap-2"><span class="bg-blue-100 text-blue-800 rounded-full min-w-[24px] h-6 inline-flex items-center justify-center text-xs font-bold px-1">${idx+1}</span><span class="flex-1">${highlightedTitle}</span></h3><div class="flex items-center gap-2">${voiceQBtn}${aiExplainBtn}<button class="hint-btn text-xs bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded transition" data-qidx="${idx}"><i class="fas fa-lightbulb"></i> Hint</button><button class="flag-btn text-sm" data-qidx="${idx}">${flagIcon}</button></div></div><div class="mt-3 space-y-1">${optionsHtml}</div>${feedback}${aiExplainBox}</div></div>`;
}
...
async function explainWithAI(idx, lengthMode = 'normal') {
    ...
    if (explainDiv.dataset.loaded === 'true' && !arguments[1]) { if (explainDiv.classList.contains('hidden')) { explainDiv.classList.remove('hidden'); aiBtn.innerHTML = `<i class="fas fa-eye-slash"></i> Ẩn giải thích`; } else { explainDiv.classList.add('hidden'); aiBtn.innerHTML = `<i class="fas fa-magic"></i> Hiện giải thích`; } return; }
    explainDiv.classList.remove('hidden'); aiBtn.innerHTML = `<i class="fas fa-eye-slash"></i> Ẩn giải thích`;
    ...
    const q = currentQuestions[idx]; const correctText = q.correctIndices.map(i => q.options[i]).join(', '); const allOptionsText = q.options.map((opt, i) => `${String.fromCharCode(65+i)}. ${opt}`).join('\n');
    let lengthPrompt = "ngắn gọn, súc tích, trực diện và dễ hiểu"; if (lengthMode === 'short') lengthPrompt = "cực kỳ ngắn gọn, tóm tắt ý chính trong 1-2 câu"; if (lengthMode === 'long') lengthPrompt = "rất chi tiết, phân tích cặn kẽ tại sao đáp án đúng lại đúng, tại sao các phương án khác lại sai, cho ví dụ minh họa nếu cần thiết";
    ...
    let data = null; try { const payload = { contents: [{ role: "user", parts: [{ text: userQuery }] }] }; data = await callAiProxy({ provider: 'google', model: 'gemini-1.5-flash', payload }); } catch (error) { explainDiv.innerHTML = `<div class="p-3 text-red-500"><i class="fas fa-exclamation-triangle mr-1"></i> AI đang bận hoặc lỗi: ${escapeHtml(error.message)}. <button onclick="explainWithAI(${idx}, '${lengthMode}')" class="underline ml-2 text-blue-600">Thử lại ngay</button></div>`; return; }
    if (data && data.candidates && data.candidates[0].content) {
        const aiText = data.candidates[0].content.parts[0].text; const formattedText = escapeHtml(aiText).replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\*(.*?)\*/g, '<i>$1</i>').replace(/\n/g, '<br>');
        const cleanAiText = aiText.replace(/[`$]/g, '\\$&');
        const toolbarHtml = `
            <div class="mt-4 pt-3 border-t border-purple-200 dark:border-slate-600 flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    <button class="ai-action-btn ai-action-btn-copy bg-blue-50 text-blue-700 px-2 py-1 rounded flex items-center gap-1 hover:bg-blue-100" onclick="copyToClipboard(\`${cleanAiText}\`)"><i class="fas fa-copy"></i> Copy</button>
                    <button class="ai-action-btn ai-action-btn-listen bg-purple-50 text-purple-700 px-2 py-1 rounded flex items-center gap-1 hover:bg-purple-100" onclick="speak(\`${cleanAiText}\`)"><i class="fas fa-volume-up"></i> Nghe</button>
                </div>
                <div class="flex items-center gap-2">
                    <button class="ai-length-btn text-xs px-2 py-1 rounded transition ${lengthMode==='short'?'bg-purple-600 text-white':'bg-white dark:bg-slate-800 text-purple-600 border border-purple-200'}" data-qidx="${idx}" data-len="short">Ngắn</button>
                    <button class="ai-length-btn text-xs px-2 py-1 rounded transition ${lengthMode==='normal'?'bg-purple-600 text-white':'bg-white dark:bg-slate-800 text-purple-600 border border-purple-200'}" data-qidx="${idx}" data-len="normal">Vừa</button>
                    <button class="ai-length-btn text-xs px-2 py-1 rounded transition ${lengthMode==='long'?'bg-purple-600 text-white':'bg-white dark:bg-slate-800 text-purple-600 border border-purple-200'}" data-qidx="${idx}" data-len="long">Chi tiết</button>
                </div>
            </div>`;
        explainDiv.innerHTML = `<div class="p-4"><div class="font-bold text-purple-800 dark:text-purple-400 mb-2 flex items-center gap-2"><i class="fas fa-robot text-lg"></i> AI Tutor Giải thích</div><div class="ai-text-content">${formattedText}</div>${toolbarHtml}</div>`;
        explainDiv.dataset.loaded = 'true'; 
    } else { explainDiv.innerHTML = `<span class="text-red-500 p-3 block"><i class="fas fa-exclamation-triangle"></i> Lỗi phân tích.</span>`; }
}
